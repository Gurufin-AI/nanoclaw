...
function createSchema(database: Database.Database): void {
  ...
  // Add image_file column to messages if it doesn't exist
  try {
    database.exec(`ALTER TABLE messages ADD COLUMN image_file TEXT`);
  } catch {
    /* column already exists */
  }
...
export function storeMessage(msg: NewMessage): void {
  db.prepare(
    `INSERT OR REPLACE INTO messages (id, chat_jid, sender, sender_name, content, timestamp, is_from_me, is_bot_message, media_kind, media_name, media_file, image_file, reply_to_message_id, reply_to_message_content, reply_to_sender_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    msg.id,
    msg.chat_jid,
    msg.sender,
...
export function getNewMessages(
  ...
  const sql = `
    SELECT * FROM (
      SELECT rowid AS _rowid, id, chat_jid, sender, sender_name, content, timestamp, is_from_me, media_kind, media_name, media_file, image_file, reply_to_message_id, reply_to_message_content, reply_to_sender_name
      FROM messages
...
export function setRegisteredGroup(jid: string, group: RegisteredGroup): void {
  ...
  db.prepare(
    `INSERT OR REPLACE INTO registered_groups (jid, name, folder, trigger_pattern, added_at, channel, container_config, requires_trigger, is_main)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    jid,
    group.name,
