...
// Force IPv4 preferentially for stable connections to Telegram
    ...
    try {
      ...
      if (messages.length > 0) {
        ...
        for (const [chatJid, groupMessages] of messagesByGroup) {
          ...
          if (ANTHROPIC_BASE_URL) {
            ...
            if (visionMessage && visionMessage.image_file) {
              ...
              try {
                ...
                // No continue here: let the regular agent handle the response now that it has the analysis
              } catch (err) {
          ...
          logger.info(
            { chatJid, promptLength: formatted.length },
            'Piping messages to active container',
          );
