...
1. Read the SQLite `registered_groups` table
2. Remove the entry for that group
3. The group folder and its files remain (don't delete them)

### Listing Groups

Read the SQLite `registered_groups` table and format it nicely.
...
When scheduling tasks for other groups, use the `target_group_jid` parameter with the group's JID from `registered_groups` table:
...
**Capabilities**:
- Receive images via Telegram and WhatsApp
- Save images to `/workspace/group/media/`
- Analyze with multimodal model

**How it works**:
1. User sends image to Telegram/WhatsApp
