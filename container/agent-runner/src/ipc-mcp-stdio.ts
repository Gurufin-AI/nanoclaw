...
const channel = process.env.NANOCLAW_CHANNEL || 'unknown';
...
• once: Local time WITHOUT "Z" suffix (e.g., "2026-02-01T15:30:00"). Do NOT use UTC/Z suffix.`,
  {
    ...
    schedule_value: z
      .string()
      .describe(
        `cron: LOCAL time expression like "0 22 * * *" (10pm local) — hours are local, never UTC-converted | interval: milliseconds like "300000" | once: local timestamp like "2026-02-01T15:30:00" (no Z suffix!)`,
      ),
    context_mode: z
      .enum(['group', 'isolated', 'task-scoped'])
      .default('group')
      .describe(
        'group=runs with chat history and memory, isolated=fresh session, task-scoped=fresh session with previous run summary and a few recent user messages injected',
      ),
    target_group_jid: z
      .string()
      .optional()
...
server.tool(
  ...
  {
    tweet_url: z.string().describe('Tweet URL to reply to'),
...
server.tool(
  ...
  { tweet_url: z.string().describe('Tweet URL to retweet') },
  async (args) => {
    if (!isMain) return { content: [{ type: 'text' as const, text: 'Only the main group can interact with X.' }], isError: true };
...
server.tool(
  ...
  {
    tweet_url: z.string().describe('Tweet URL to quote'),
